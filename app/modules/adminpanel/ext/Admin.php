 ...
 */
namespace app\modules\adminpanel\ext;

class Admin
{

    function __construct( $Main, $Modules, $DataBase/*, $Auth,  $Translate*/) {
    ...
    function action_clear_all_cache() {
        ...
        file_exists( $cache_files['modules_cache'] ) && print('modules_cache имеется ');

        // Очищаем кэш каждого модуля.
        for ( $i = 0; $i < $this->Modules->modules_count; $i++ ):
            $module = array_keys( $this->Modules->modules )[ $i ];
    ...
    public function GetArrModules()
    {
        ...
        $modules_list = array_diff( scandir( MODULES, 1 ), array( '..', '.', '_disabled' ) );
    ...
    function edit_modules_initialization() {
        ...
        $array = $this->Modules->module_init;
    ...
    function edit_module() {
        ...
        $modules_init = $this->Modules->module_init;
    ...
    function add_anime($name, $alt_name, $link, $image, $desc) {
        ...
        return $this->DataBase->query("INSERT INTO anime(`name`, `alt_name`, `link`, `image`, `description`) VALUES('$name', '$alt_name', '$link', ".'"'.$image.'"'.", '$desc')");
    }
}